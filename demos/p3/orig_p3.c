/*
 * orig_p3.c: This file is part of the IEC project.
 *
...
int *ia_cf,*ia_cv,*lcf,*lcv;
int *lfc0,*lfc1,*lfv0,*lfv1;
...
void print_output()
{
  ...
  for( i = 0 ; i < ncells ; i++ )
    fprintf(outfile,"C:%d %lf %lf %lf %lf\n",i,phic_new[i][0],phic_new[i][1],phic_new[i][2],phic_new[i][3]);
  for (i = 0; i < nnodes; i++) 
    fprintf(outfile,"V:%d %lf %lf %lf %lf\n",i,phiv[i][0],phiv[i][1],phiv[i][2],phiv[i][3]);
  for (i = 0; i < nfaces; i++) 
    fprintf(outfile,"F:%d %lf %lf %lf %lf\n",i,dphi[i][0],dphi[i][1],dphi[i][2],dphi[i][3]);
  for (i = 0; i < nbcfaces; i++) 
    fprintf(outfile,"B:%d %lf %lf %lf %lf\n",i,phib[i][0],phib[i][1],phib[i][2],phib[i][3]);
  fclose(outfile);
}
