...
/* Time integration loop */
  ...
  int j_1,j_2,i_1,i_2,i_3,j_3,i_4,j_4,i_5,j_5,i_6,j_6,i_7,j_7,i_8,j_8,i_9,j_9,i_10,j_10,i_11,j_11,i_12,j_12,i_13,j_13,i_14,j_14,i_15,j_15;
  int Anext_2,col_2,Anext_7,col_7,Anext_12,col_12;

  double start_t = rtclock();
  for (iter = 1; iter <= timesteps ; ) {

    for (i_1 = 0; i_1 < ARCHnodes; i_1++)
      for (j_1 = 0; j_1 < 3; j_1++)
        disp0[i_1][j_1] = 0.0;
    ...
    for (i_2 = 0; i_2 < ARCHnodes; i_2++) {
      ...
      sum0 = K[Anext_2][0]*disp1[i_2][0] + K[Anext_2][1]*disp1[i_2][1] + K[Anext_2][2]*disp1[i_2][2];
      sum1 = K[Anext_2][3]*disp1[i_2][0] + K[Anext_2][4]*disp1[i_2][1] + K[Anext_2][5]*disp1[i_2][2];
      sum2 = K[Anext_2][6]*disp1[i_2][0] + K[Anext_2][7]*disp1[i_2][1] + K[Anext_2][8]*disp1[i_2][2];

      for( j_2 = ARCHmatrixindex[i_2]+1 ; j_2 < ARCHmatrixindex[i_2+1] ; j_2++){
    	col_2 = ARCHmatrixcol[j_2];
    	...
    	disp0[col_2][2] += K[j_2][2]*disp1[i_2][0] + K[j_2][5]*disp1[i_2][1] + K[j_2][8]*disp1[i_2][2];
      }
      disp0[i_2][0] += sum0;
      disp0[i_2][1] += sum1;
      disp0[i_2][2] += sum2;
    ...
    for (i_3 = 0; i_3 < ARCHnodes; i_3++)
      for (j_3 = 0; j_3 < 3; j_3++)
        disp0[i_3][j_3] = (-disp0[i_3][j_3] * Exc.dt * Exc.dt ) +  2.0 * M[i_3][j_3] * disp1[i_3][j_3] -
    	  (M[i_3][j_3] - Exc.dt / 2.0 * C[i_3][j_3]) * disp2[i_3][j_3] -
    	    Exc.dt * Exc.dt * (M23[i_3][j_3] * phi2(time) / 2.0 +
    ...
    for (i_5 = 0; i_5 < ARCHnodes; i_5++)
      for (j_5 = 0; j_5 < 3; j_5++)
        vel[i_5][j_5] = 0.5 / Exc.dt * (disp0[i_5][j_5] -
    				    disp2[i_5][j_5]);
    ...
    for (i_7 = 0; i_7 < ARCHnodes; i_7++) {
      ...
      sum0 = K[Anext_7][0]*disp0[i_7][0] + K[Anext_7][1]*disp0[i_7][1] + K[Anext_7][2]*disp0[i_7][2];
      sum1 = K[Anext_7][3]*disp0[i_7][0] + K[Anext_7][4]*disp0[i_7][1] + K[Anext_7][5]*disp0[i_7][2];
    ...
    for (i_8 = 0; i_8 < ARCHnodes; i_8++)
      for (j_8 = 0; j_8 < 3; j_8++)
        disp2[i_8][j_8] = (-disp2[i_8][j_8] * Exc.dt * Exc.dt ) +  2.0 * M[i_8][j_8] * disp0[i_8][j_8] -
    	  (M[i_8][j_8] - Exc.dt / 2.0 * C[i_8][j_8]) * disp1[i_8][j_8] -
    	    Exc.dt * Exc.dt * (M23[i_8][j_8] * phi2(time) / 2.0 +
    ...
    for (i_10 = 0; i_10 < ARCHnodes; i_10++)
      for (j_10 = 0; j_10 < 3; j_10++)
        vel[i_10][j_10] = 0.5 / Exc.dt * (disp2[i_10][j_10] -
    				    disp1[i_10][j_10]);
    ...
    for (i_12 = 0; i_12 < ARCHnodes; i_12++) {
      ...
      sum0 = K[Anext_12][0]*disp2[i_12][0] + K[Anext_12][1]*disp2[i_12][1] + K[Anext_12][2]*disp2[i_12][2];
      sum1 = K[Anext_12][3]*disp2[i_12][0] + K[Anext_12][4]*disp2[i_12][1] + K[Anext_12][5]*disp2[i_12][2];
      sum2 = K[Anext_12][6]*disp2[i_12][0] + K[Anext_12][7]*disp2[i_12][1] + K[Anext_12][8]*disp2[i_12][2];

      for( j_12 = ARCHmatrixindex[i_12]+1 ; j_12 < ARCHmatrixindex[i_12+1] ; j_12++){
    	col_12 = ARCHmatrixcol[j_12];
    	...
    	disp1[col_12][2] += K[j_12][2]*disp2[i_12][0] + K[j_12][5]*disp2[i_12][1] + K[j_12][8]*disp2[i_12][2];
      }
      disp1[i_12][0] += sum0;
      disp1[i_12][1] += sum1;
      disp1[i_12][2] += sum2;
    ...
    for (i_13 = 0; i_13 < ARCHnodes; i_13++)
      for (j_13 = 0; j_13 < 3; j_13++)
        disp1[i_13][j_13] = (-disp1[i_13][j_13] * Exc.dt * Exc.dt ) +  2.0 * M[i_13][j_13] * disp2[i_13][j_13] -
    	  (M[i_13][j_13] - Exc.dt / 2.0 * C[i_13][j_13]) * disp0[i_13][j_13] -
    	    Exc.dt * Exc.dt * (M23[i_13][j_13] * phi2(time) / 2.0 +
    ...
    for (i_15 = 0; i_15 < ARCHnodes; i_15++)
      for (j_15 = 0; j_15 < 3; j_15++)
        vel[i_15][j_15] = 0.5 / Exc.dt * (disp1[i_15][j_15] -
    				    disp0[i_15][j_15]);
...
/*     disptplus = i; */
  ...
  fprintf(stderr,"[IEC]:OriginalTime:%lf\n",stop_t-start_t);
