...
// SideEffectAnalysis* SageTools::rose_sideeffect = SideEffectAnalysis::create();
// bool SageTools::SideEffectAnalysis_done = false;
  ...
  public:
    ...
    virtual void postOrderVisit(SgNode* node) {
      ...
      if (fe && (!_allowMathFuncs ||
		 ! isMathFunc(fe->getAssociatedFunctionSymbol(),call_stack)))
	_is_error = true;
      if (! _is_error &&
	  ...
	  case V_SgFunctionCallExp:
	    {
	      ...
	      std::vector<SgFunctionSymbol*> call_stack;
	      if (! _allowMathFuncs ||
		  ! isMathFunc(f->getAssociatedFunctionSymbol(),call_stack))
		_is_error = true;
	    }
	    break;
...
// void 
// SageTools::doSideEffectAnalysis(SgProject& project)
// {
//   rose_sideeffect->calcSideEffect(project);
//   sideEffectAnalysis_done = true;
// }
 ...
 */
bool
SageTools::isMathFunc(SgFunctionSymbol* fs, std::vector<SgFunctionSymbol*>& call_stack)
{
  if (fs)
    {
	...
	return true;
      else if( symb == "printf" || symb == "fprintf" || symb == "exit" ||
	       symb == "assert" || symb == "fscanf" || symb == "scanf" ||
	       symb == "fopen" || symb == "fclose" || symb == "malloc" ||
	       symb == "free" || symb == "fflush" )
	return false;
      //Using the ROSE sideeffect analyser. Put this back later
      // else{
      // 	std::list<const char*> side_effect_info = rose_sideeffect->getGMOD(symb) ;
      // 	printf("[SideEffect] Func: %s\n",symb.c_str());
      // 	for( std::list::const_iterator it = side_effect_info.begin() ; it != side_effect_info.end() ; it++ )
      // 	  printf("[SideEffect] %s\n",(*it).c_str());
      // }

      //For now using a simple analysis. HT : Martin Kong
      else{
	SgFunctionDeclaration* fs_dec = fs->get_declaration();
	if( fs_dec ){
	  SgInitializedNamePtrList& fs_args=  fs_dec->get_args();
	  SgInitializedNamePtrList::iterator iter = fs_args.begin();
	  bool checkTypes = true;
	  for( ; iter != fs_args.end() ; iter++ ){
	    SgType* arg_type = (*iter)->get_type();
	    if( !(isSgTypeBool(arg_type) || isSgTypeChar(arg_type) ||
		  isSgTypeDouble(arg_type) || isSgTypeFloat(arg_type) ||
		  isSgTypeInt(arg_type) || isSgTypeLong(arg_type) ||
		  isSgTypeLongDouble(arg_type) || isSgTypeLongLong(arg_type) ||
		  isSgTypeShort(arg_type) || isSgTypeSignedInt(arg_type) ||
		  isSgTypeSignedLong(arg_type) || isSgTypeSignedLongLong(arg_type) ||
		  isSgTypeSignedShort(arg_type) || isSgTypeString(arg_type) ||
		  isSgTypeUnsignedChar(arg_type) || isSgTypeUnsignedInt(arg_type) ||
		  isSgTypeUnsignedLong(arg_type) || isSgTypeUnsignedLongLong(arg_type) ||
		  isSgTypeUnsignedShort(arg_type) || isSgArrayType(arg_type) ) ){
	      checkTypes = false;
	      break;
	    }	    
	  }
	  if( checkTypes){
	    ...
	    for( std::vector<SgFunctionSymbol*>::iterator it = call_stack.begin() ; it != call_stack.end() ; it++ )
	      if( (*it) == fs ){
		is_recursive = true;
		break;
	      }
	    if( is_recursive )
	      ...
	      call_stack.push_back(fs);
	      return checkFunctionBody(fs_dec,call_stack);
	    }
	  }
	  else
	    return false;
...
bool 
SageTools::checkFunctionBody(SgFunctionDeclaration* fn_decl, std::vector<SgFunctionSymbol*>& call_stack)
{
  SgDeclarationStatement* fn_defn_decl = fn_decl->get_definingDeclaration();
  if( fn_defn_decl){
    if( !isSgFunctionDeclaration(fn_defn_decl) )
      return false;
    SgFunctionDefinition* fn_defn = isSgFunctionDeclaration(fn_defn_decl)->get_definition();
    std::vector<SgVarRefExp*> all_vars = querySubTree<SgVarRefExp>(fn_defn->get_body()) ;
    bool all_local = true;
    for( std::vector<SgVarRefExp*>::iterator it = all_vars.begin() ; it != all_vars.end() ; it++ ){
      SgVariableSymbol* curr_symbol = (*it)->get_symbol();
      SgScopeStatement* curr_decl = curr_symbol->get_declaration()->get_scope();
      if( curr_decl == NULL || isSgGlobal(curr_decl) ){
	all_local = false;
	break;
      }
    }
    if( all_local ){
      std::vector<SgFunctionCallExp*> all_funcs = querySubTree<SgFunctionCallExp>(fn_defn->get_body());
      bool math_funcs = true;
      for( std::vector<SgFunctionCallExp*>::iterator it = all_funcs.begin() ; it != all_funcs.end() ; it++ ){
	SgFunctionSymbol* fs = (*it)->getAssociatedFunctionSymbol();
	if( !isMathFunc(fs,call_stack) ){
	  math_funcs = false;
	  break;
	}
      }
