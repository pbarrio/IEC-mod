...
void codegen::GenerateCCode(SgGlobal* global_scope)
{
  ...
  if( CompilerOpts::IsOpenMPMode() ){ 
    ...
    // SgVariableSymbol* nthreads_decl = global_scope->lookup_variable_symbol("__nthreads__");
    // assert(nthreads_decl);
  ...
  // SgNullStatement* executor_stmt = SageBuilder::buildNullStatement();
  // SageInterface::insertStatementBefore(all_stmts.front(),executor_stmt);
  ...
  for( deque<partitionable_loop*>::iterator it = all_loops.begin() ; it != all_loops.end() ; it++ ){
    partitionable_loop* curr_part_loop = *it;
    //curr_part_loop->ResetAllCounters(init_thread_counter);
  ...
  if( CompilerOpts::IsOpenMPMode() ){
    //SageInterface::replaceStatement(executor_stmt,root_node);
