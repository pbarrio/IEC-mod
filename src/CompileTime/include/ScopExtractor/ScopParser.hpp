...
public:
  ...
  /**
   * Return true if the expression is an affine form of the symbols,
   * extended to include array access expressions
   * referenced in the 'symbols' set.
   *
   */
  bool isBoundExtendedAffineExpr(SgExpression* expr,
		    std::set<SgVariableSymbol*>& symbols);
  ...
  bool parseExtendedAffineExpression(SgExpression* expr);
  ...
  bool parseStatementExpression(SgExpression* node);
